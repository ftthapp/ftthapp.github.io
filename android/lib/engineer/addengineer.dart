...
class AddEngineer extends StatefulWidget {
  ...
  @override
  State<AddEngineer> createState() => _AddEngineerState();
}

class _AddEngineerState extends State<AddEngineer> {
  ...
  Future<void> _addPastor() async {
    ...
    try {
      ...
      String name = '${selectedParentDoc['name']}';
      String email = selectedParentDoc['email'];

      // Generate a unique document ID for Firestore
      String documentId = FirebaseFirestore.instance.collection('engineer').doc().id;

      // Add Pastor to Firestore
      await FirebaseFirestore.instance.collection('engineer').doc(documentId).set({
        'availability': true,
        'name': name,
        'email': email,
      ...
      // Add Pastor to MySQL via PHP API
      final url = Uri.parse('http://localhost/projects/ftth/addengineer.php'); // Update with your PHP API URL
      ...
      if (response.statusCode == 200) {
        ...
        if (result['success'] == true) {
          _showSuccessDialog("Pastor added successfully!");
        } else {
          _showErrorDialog("Failed to add Engineer to MySQL: ${result['message']}");
        }
      } else {

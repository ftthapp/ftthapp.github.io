...
class AddAreaTeamLead extends StatefulWidget {
  const AddAreaTeamLead({Key? key}) : super(key: key);

  @override
  State<AddAreaTeamLead> createState() => _AddAreaTeamLeadState();
}

class _AddAreaTeamLeadState extends State<AddAreaTeamLead> {
  ...
  Future<void> _addPastor() async {
    ...
    try {
      ...
      String documentId = FirebaseFirestore.instance.collection('areateamlead').doc().id;

      // Add Area Team Lead to Firestore
      await FirebaseFirestore.instance.collection('areateamlead').doc(documentId).set({
        'availability': true,
        'name': name,
        'email': email,
      ...
      // Optionally Add Area Team Lead to MySQL via PHP API
      final url = Uri.parse('http://localhost/projects/ftth/addareateamlead.php'); // Update with your PHP API URL
      ...
      if (response.statusCode == 200) {
        ...
        if (result['success'] == true) {
          _showSuccessDialog("Area Team Lead added successfully!");
        } else {
          _showErrorDialog("Failed to add Area Team Lead to MySQL: ${result['message']}");
        }
      } else {

...
import 'addareateamlead.dart';

class AreaTeamLeadTab extends StatefulWidget {
  const AreaTeamLeadTab({Key? key}) : super(key: key);

  @override
  State<AreaTeamLeadTab> createState() => _AreaTeamLeadTabState();
}

class _AreaTeamLeadTabState extends State<AreaTeamLeadTab> {
  ...
  void _showDeleteDialog(String memberId) {
    showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          title: const Text('Delete Area Team Lead?'),
          content: const Text('Are you sure you want to delete this Area Team Lead?'),
          actions: [
            TextButton(
  ...
  Future<void> _deleteMember(String memberId) async {
    try {
      await FirebaseFirestore.instance.collection('areateamlead').doc(memberId).delete();
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: TextField(
          ...
          decoration: const InputDecoration(
            hintText: "Type Area Team Lead name",
        ...
        actions: [
          ...
          TextButton(
            onPressed: () {
              Navigator.push(
                context,
                MaterialPageRoute(builder: (context) => AddAreaTeamLead()),
              );
            },
      ...
      body: StreamBuilder<QuerySnapshot>(
        stream: FirebaseFirestore.instance.collection('areateamlead').snapshots(),
        builder: (context, snapshot) {
          if (snapshot.connectionState == ConnectionState.waiting) {
            return const Center(child: CircularProgressIndicator());
          }
          if (!snapshot.hasData || snapshot.data!.docs.isEmpty) {
            return const Center(child: Text('No AreaTeamLead found'));
          }

...
class _AddSupportState extends State<AddSupport> {
  ...
  Future<void> _addPastor() async {
    ...
    try {
      ...
      String documentId = FirebaseFirestore.instance.collection('support').doc().id;

      // Add Support to Firestore
      await FirebaseFirestore.instance.collection('support').doc(documentId).set({
        'availability': true,
        'name': name,
        'email': email,
      ...
      // Optionally Add Support to MySQL via PHP API (if needed)
      final url = Uri.parse('http://localhost/projects/ftth/addsupport.php'); // Update with your PHP API URL
      ...
      if (response.statusCode == 200) {
        ...
        if (result['success'] == true) {
          _showSuccessDialog("Support added successfully!");
        } else {
          _showErrorDialog("Failed to add Support to MySQL: ${result['message']}");
        }
      } else {

...
import 'addcvmanalyst.dart';

class CvmAnalystTab extends StatefulWidget {
  const CvmAnalystTab({Key? key}) : super(key: key);

  @override
  State<CvmAnalystTab> createState() => _CvmAnalystTabState();
}

class _CvmAnalystTabState extends State<CvmAnalystTab> {
  ...
  void _showDeleteDialog(String memberId) {
    showDialog(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          title: const Text('Delete CVM Analyst?'),
          content: const Text('Are you sure you want to delete this CVM Analyst?'),
          actions: [
            TextButton(
  ...
  Future<void> _deleteMember(String memberId) async {
    try {
      await FirebaseFirestore.instance.collection('cvmanalyst').doc(memberId).delete();
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: TextField(
          ...
          decoration: const InputDecoration(
            hintText: "Type CVM Analyst name",
        ...
        actions: [
          ...
          TextButton(
            onPressed: () {
              Navigator.push(
                context,
                MaterialPageRoute(builder: (context) => AddCvmAnalyst()),
              );
            },
      ...
      body: StreamBuilder<QuerySnapshot>(
        stream: FirebaseFirestore.instance.collection('cvmanalyst').snapshots(),
        builder: (context, snapshot) {
          if (snapshot.connectionState == ConnectionState.waiting) {
            return const Center(child: CircularProgressIndicator());
          }
          if (!snapshot.hasData || snapshot.data!.docs.isEmpty) {
            return const Center(child: Text('No CVM Analyst found'));
          }
